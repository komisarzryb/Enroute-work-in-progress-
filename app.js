const screens=document.querySelectorAll(".screen");document.querySelectorAll("nav button").forEach(b=>b.onclick=()=>{screens.forEach(s=>s.classList.remove("active"));document.getElementById(b.dataset.screen).classList.add("active");if(b.dataset.screen==="map"){if(typeof initMap==="function"&&!mapInitialized)initMap();if(typeof refreshMap==="function")setTimeout(refreshMap,100)}});function add(t,m){let[a,b]=t.split(":").map(Number),x=a*60+b+m;return String(Math.floor(x/60)%24).padStart(2,"0")+":"+String(x%60).padStart(2,"0")}document.getElementById("plan").onclick=()=>{let t=document.getElementById("schoolTime").value||"09:40";document.getElementById("leave").textContent="Wyjdź o "+add(t,-88);document.getElementById("morning").textContent="Przyjazd "+add(t,-16)+" · 16 min zapasu"};document.getElementById("return").onclick=()=>{let t=document.getElementById("endTime").value||"15:25";document.getElementById("returnResult").textContent="Proponowany wyjazd: "+add(t,8)+" · przyjazd około "+add(t,48)};document.getElementById("snake").onclick=()=>alert("Snake — dodamy pełną minigrę w kolejnej wersji.");
